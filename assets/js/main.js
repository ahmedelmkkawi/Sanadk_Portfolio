/**
 * Sanadak Portfolio — Main Native JavaScript
...
function getPortfolioData() {
  const team = typeof TEAM_DATA !== 'undefined' ? TEAM_DATA : (typeof PORTFOLIO_DATA !== 'undefined' && PORTFOLIO_DATA.team ? PORTFOLIO_DATA.team : []);
...
function createProjectCardHTML(project, index = 0) {
  const techBadges = (project.technologies || [])
    .slice(0, 4)
    .map(tech => `<span class="text-xs bg-white/10 text-white px-2.5 py-1 rounded-md border border-white/5 font-medium">${tech}</span>`)
    .join('');

  let teamAvatarHTML = '';
  if (project.teamMembers && project.teamMembers.length > 0) {
    const firstMember = project.teamMembers[0];
    const name = typeof firstMember === 'object' ? firstMember.name : firstMember;
    const avatar = typeof firstMember === 'object' && firstMember.avatar ? firstMember.avatar : null;
...
function initHomePage() {
  ...
  const featuredProjects = data.projects.filter(p => p.featured === true || p.featured === "true");
  ...
  if (teamContainer && data.team) {
    ...
    teamContainer.innerHTML = data.team.map((member, idx) => {
      ...
      const titleColorClass = isWhiteSection ? "text-slate-900" : "text-white";
      const roleColorClass = isWhiteSection ? "text-[#781e1e]" : "text-amber-500";
      const badgeBgClass = isWhiteSection ? "bg-slate-100 text-slate-700 border-slate-200" : "bg-white/10 text-white border-white/5";
      const iconBorderClass = isWhiteSection ? "border-slate-300 bg-slate-100 text-slate-800 hover:bg-slate-200" : "border-white/20 bg-white/10 text-white hover:bg-white/20";
      const bioGradientClass = isWhiteSection ? "from-slate-900/90 via-slate-900/40" : "from-black/95 via-black/40";
          ...
          <div class="flex flex-col justify-between flex-grow">
            <div>
              ...
              <div class="flex flex-wrap gap-1 mb-3">
                ${(member.skills || []).slice(0, 4).map(s => `<span class="text-[9px] font-medium ${badgeBgClass} px-2 py-0.5 rounded border">${s}</span>`).join('')}
              </div>
            </div>
              ...
              ${member.activeProjects && member.activeProjects.length > 0 ? `
                <div>
                  ...
                  <div class="flex flex-wrap gap-1">
                    ${member.activeProjects.map(p => `<span class="text-[9px] ${badgeBgClass} px-2 py-0.5 rounded border font-medium line-clamp-1">${p}</span>`).join('')}
                  </div>
                </div>
              ` : ''}
              ...
              <div class="flex items-center justify-between pt-1">
                ...
                <div class="flex gap-1.5">
                  ${member.socialLinks && member.socialLinks.linkedin ? `
                    <a href="${member.socialLinks.linkedin}" target="_blank" class="w-7 h-7 rounded-full border ${iconBorderClass} flex items-center justify-center transition-all shadow text-[10px]" title="LinkedIn"><i class="fab fa-linkedin-in"></i></a>
                  ` : ''}
                  ${member.socialLinks && member.socialLinks.github ? `
                    <a href="${member.socialLinks.github}" target="_blank" class="w-7 h-7 rounded-full border ${iconBorderClass} flex items-center justify-center transition-all shadow text-[10px]" title="GitHub"><i class="fab fa-github"></i></a>
                  ` : ''}
                  ${member.socialLinks && member.socialLinks.portfolio ? `
                    <a href="${member.socialLinks.portfolio}" target="_blank" class="w-7 h-7 rounded-full border ${iconBorderClass} flex items-center justify-center transition-all shadow text-[10px]" title="Portfolio"><i class="fas fa-globe"></i></a>
                  ` : ''}
                </div>
              </div>
            </div>
...
function initFeaturedCarousel(featuredProjects) {
  ...
  if (dotsContainer) {
    dotsContainer.innerHTML = featuredProjects.map((_, idx) => `
...
function initProjectsPage() {
  ...
  function filterAndRender() {
    const filtered = (data.projects || []).filter(project => {
      // Category filter
      const matchesCat = activeCategory === 'all' || (project.category && project.category.toLowerCase() === activeCategory.toLowerCase());
      
      // Technology filter
      const matchesTech = activeTech === 'all' || (project.technologies && project.technologies.some(t => t.toLowerCase() === activeTech.toLowerCase()));
    ...
    if (filtered.length > 0) {
      container.innerHTML = filtered.map((p, idx) => createProjectCardHTML(p, idx)).join('');
    } else {
      container.innerHTML = `
...
function initProjectDetailPage() {
  ...
  document.title = `${project.title} — Sanadak Portfolio`;

  const categoryElem = document.getElementById('detailCategory');
  if (categoryElem) categoryElem.textContent = project.categoryLabel || project.category;
  ...
  if (trackElem && screenshots.length > 0) {
    // Render Carousel Slides
    trackElem.innerHTML = screenshots.map((src, idx) => `
    ...
    if (thumbsElem) {
      if (screenshots.length > 1) {
        ...
        thumbsElem.innerHTML = screenshots.map((src, idx) => `
  ...
  if (roleElem) roleElem.textContent = project.role || "Development & Integration";

  // Participating Team Members
  const teamElem = document.getElementById('detailTeamMembers');
  if (teamElem) {
    if (project.teamMembers && project.teamMembers.length > 0) {
      teamElem.innerHTML = project.teamMembers.map(member => {
        const name = typeof member === 'object' ? member.name : member;
        const role = typeof member === 'object' && member.role ? member.role : 'Contributor';
        const avatar = typeof member === 'object' && member.avatar ? member.avatar : null;
        return `
  ...
  if (roleListElem && project.roleDetails) {
    roleListElem.innerHTML = project.roleDetails.map(item => `
  ...
  if (techElem && project.technologies) {
    techElem.innerHTML = project.technologies.map(t => `<span class="tech-tag"><i class="fas fa-code text-[11px] text-amber-400 mr-1"></i>${t}</span>`).join('');
  }

  // Features List
  const featuresElem = document.getElementById('detailFeatures');
  if (featuresElem && project.features) {
    featuresElem.innerHTML = project.features.map(f => `
